...
func (inihash *Inihash) verifyHeader(chain consensus.ChainHeaderReader, header, parent *types.Header, uncle bool, seal bool, unixNow int64) error {
	...
	if inihash.config.StartBlockHeight > 0 {
		isIn := false
		if big.NewInt(int64(inihash.config.StartBlockHeight)).Cmp(header.Number) == -1 {

			for _, addr := range inihash.config.AllowList {
...
func (inihash *Inihash) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state vm.StateDB, txs *[]*types.Transaction,
	...
	signer := types.MakeSigner(inihash.chainConfig, header.Number, header.Time)
	for _, oneTx := range *txs {

		acc, _ := types.Sender(signer, oneTx)
		ban, exists := inihash.config.BlackList[acc]
		if exists && ban {
			return consensus.ErrUnknownAncestor
		}
	}
