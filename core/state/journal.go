...
func (j *journal) lockBalanceChange(addr common.Address, previous *uint256.Int) {
	j.append(lockBalanceChange{
		account: &addr,
		prev:    previous.Clone(),
	})
}
...
func (ch lockBalanceChange) copy() journalEntry {
	return lockBalanceChange{
		account: ch.account,
