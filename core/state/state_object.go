...
func (s *stateObject) empty() bool {
	return s.data.Nonce == 0 && s.data.Balance.IsZero() && s.data.LockBalance.IsZero() && bytes.Equal(s.data.CodeHash, types.EmptyCodeHash.Bytes())
}
...
// It is used to add funds to the destination account of a transfer.
func (s *stateObject) AddLockBalance(amount *uint256.Int) uint256.Int {
	// EIP161: We must check emptiness for the objects such that the account
	// clearing (0,0,0 objects) can take effect.
	if amount.Sign() == 0 {
		if s.empty() {
		...
		return *(s.LockBalance())
	}
	return s.SetLockBalance(new(uint256.Int).Add(s.LockBalance(), amount))
}
...
// SetBalance sets the balance for the object, and returns the previous balance.
func (s *stateObject) SetLockBalance(amount *uint256.Int) uint256.Int {
	prev := *s.data.LockBalance
	s.db.journal.lockBalanceChange(s.address, s.data.LockBalance)
	s.setLockBalance(amount)
	return prev
}
func (s *stateObject) setLockBalance(amount *uint256.Int) {
...
func (s *stateObject) LockBalance() *uint256.Int {
