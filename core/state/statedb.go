...
import (
	...
	"github.com/Project-InitVerse/chain/trie/trienode"
	"github.com/Project-InitVerse/chain/trie/utils"
...
func (s *StateDB) StartPrefetcher(namespace string, witness *stateless.Witness) {
	...
	// https://github.com/Project-InitVerse/chain/issues/29880
...
// GetBalance retrieves the balance from the given address or 0 if object not found
func (s *StateDB) GetLockBalance(addr common.Address) *uint256.Int {
	stateObject := s.getStateObject(addr)
	if stateObject != nil {
...
// AddBalance adds amount to the account associated with addr.
func (s *StateDB) AddLockBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason) uint256.Int {
	stateObject := s.getOrNewStateObject(addr)
	if stateObject == nil {
		return uint256.Int{}
	}
	return stateObject.AddLockBalance(amount)
}
...
// SubBalance subtracts amount from the account associated with addr.
func (s *StateDB) SubLockBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason) uint256.Int {
	stateObject := s.getOrNewStateObject(addr)
	if stateObject == nil {
		return uint256.Int{}
	}
	...
	return stateObject.SetLockBalance(new(uint256.Int).Sub(stateObject.LockBalance(), amount))
}
...
func (s *StateDB) SetLockBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason) {
	stateObject := s.getOrNewStateObject(addr)
	if stateObject != nil {
...
func (s *StateDB) SetStorage(addr common.Address, storage map[common.Hash]common.Hash) {
	...
	if obj != nil {
		...
		newObj.SetLockBalance(obj.LockBalance())
	}
}
