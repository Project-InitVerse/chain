...
func (s *hookedStateDB) GetLockBalance(addr common.Address) *uint256.Int {
	return s.inner.GetLockBalance(addr)
}
...
func (s *hookedStateDB) SetLockBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason) {
	s.inner.SetLockBalance(addr, amount, reason)
}
...
func (s *hookedStateDB) SubLockBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason) uint256.Int {
	prev := s.inner.SubLockBalance(addr, amount, reason)
...
func (s *hookedStateDB) AddLockBalance(addr common.Address, amount *uint256.Int, reason tracing.BalanceChangeReason) uint256.Int {
	prev := s.inner.AddLockBalance(addr, amount, reason)
