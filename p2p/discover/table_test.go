...
func TestTable_filterNode(t *testing.T) {
	...
	var r2 enr.Record
	r2.Set(enr.WithEntry("eth", eth{ForkID: forkid.NewID(params.MainnetChainConfig, core.DefaultGenesisBlock().ToBlock(), uint64(0), uint64(0))}))
	if enrFilter(&r2) {
	...
	var r3 enr.Record
	r3.Set(enr.WithEntry("eth", eth{ForkID: forkid.NewID(params.MainnetChainConfig, core.DefaultGenesisBlock().ToBlock(), uint64(0), uint64(0))}))
	if !enrFilter(&r3) {
...
func TestNodesPush(t *testing.T) {
	...
	for _, nodes := range perm {
		...
		if !slices.EqualFunc(list.entries, []*enode.Node(perm[0][:2]), nodeIDEqual) {
			t.Fatal("not equal")
		}
	}
}
