...
import (
	...
	"github.com/Project-InitVerse/chain/beacon/fakebeacon"
	bparams "github.com/Project-InitVerse/chain/beacon/params"
	...
	"github.com/Project-InitVerse/chain/core/txpool/blobpool"
	"github.com/Project-InitVerse/chain/core/txpool/legacypool"
	"github.com/Project-InitVerse/chain/core/vm"
	"github.com/Project-InitVerse/chain/crypto"
	"github.com/Project-InitVerse/chain/crypto/kzg4844"
	...
	"github.com/Project-InitVerse/chain/ethdb/remotedb"
	...
	"github.com/Project-InitVerse/chain/internal/version"
	...
	"github.com/Project-InitVerse/chain/miner/minerconfig"
	...
	"github.com/Project-InitVerse/chain/triedb/hashdb"
	"github.com/Project-InitVerse/chain/triedb/pathdb"
...
var (
	...
	MinerAllowAddrFlag = &cli.StringSliceFlag{
		Name:     "miner.minerallow",
		Usage:    "Only Support allow block miner",
		Category: flags.MinerCategory,
	}
	MinerStartBlockFlag = &cli.IntFlag{
		Name:     "miner.allowstartheight",
		Usage:    "miner allow active block number",
		Category: flags.MinerCategory,
	}
	MinerBlackListFlag = &cli.StringSliceFlag{
		Name:     "miner.blacklist",
		Usage:    "Public address for black list address",
		Category: flags.MinerCategory,
	}
...
var (
	...
	NetworkFlags = []cli.Flag{MainnetFlag, TestnetFlag, DevnetFlag}
...
func setInihash(ctx *cli.Context, cfg *ethconfig.Config) {
	if ctx.IsSet(MinerAllowAddrFlag.Name) {

		AllowList := ctx.StringSlice(MinerAllowAddrFlag.Name)

		AddrList := make([]common.Address, 0)
		for _, oneAddr := range AllowList {
			AddrList = append(AddrList, common.HexToAddress(oneAddr))
		}
		cfg.Inihash.AllowList = AddrList
	}
	cfg.Inihash.StartBlockHeight = 0
	if ctx.IsSet(MinerStartBlockFlag.Name) {

		cfg.Inihash.StartBlockHeight = ctx.Int(MinerStartBlockFlag.Name)
...
// setBlackList retrieves the etherbase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func setBlackList(ctx *cli.Context, cfg *ethconfig.Config) {
	// Extract the current etherbase
	if ctx.IsSet(MinerBlackListFlag.Name) {

		blackList := ctx.StringSlice(MinerBlackListFlag.Name)

		AddrList := make(map[common.Address]bool, 0)

		for _, oneAddr := range blackList {
			AddrList[common.HexToAddress(oneAddr)] = true
		}
		cfg.Miner.BlackList = AddrList
		cfg.Inihash.BlackList = AddrList
	} else {
		cfg.Miner.BlackList = make(map[common.Address]bool)
		cfg.Inihash.BlackList = make(map[common.Address]bool)
	}
}
...
func MakeChain(ctx *cli.Context, stack *node.Node, readonly bool) (*core.BlockChain, ethdb.Database) {
	...
	inihashConfig := inihash.DefaultIniConfig
	engine, err := ethconfig.CreateConsensusEngine(config, &inihashConfig, nil, false, chainDb, nil, genesisHash)
	if err != nil {
		Fatalf("%v", err)
	}
	if gcmode := ctx.String(GCModeFlag.Name); gcmode != "full" && gcmode != "archive" {
