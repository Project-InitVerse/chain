...
const (
	...
	SystemTxsGasSoftLimit uint64 = 500000   // Maximum gas reserved for system transactions, excluding validator update transactions (Parlia consensus only)
	SystemTxsGas          uint64 = 500000
...
var (
	DifficultyBoundDivisor    = big.NewInt(12288)     // The bound divisor of the difficulty, used in the update calculations.
	GenesisDifficulty         = big.NewInt(100000000) // Difficulty of the Genesis block.
	MinimumDifficulty         = big.NewInt(50000000)  // The minimum that the difficulty may ever be.
	...
	NewDurationLimit          = big.NewInt(10)
	NewDifficultyBoundDivisor = big.NewInt(2048)
)
var (
