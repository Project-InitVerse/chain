...
func (w *worker) setEtherbase(addr common.Address) {
	...
	if is_in == false {
		w.posCoinbase = append(w.posCoinbase, addr)
	}
}
	...
	for {
		...
		case <-timer.C:
			...
			if w.isRunning() && ((w.chainConfig.Ethash != nil) || (w.chainConfig.Clique != nil &&
				w.chainConfig.Clique.Period > 0) || (w.chainConfig.Dpos != nil && w.chainConfig.Dpos.Period > 0) || (w.chainConfig.Parlia != nil && w.chainConfig.Parlia.Period > 0)) {
				// Short circuit if no new transaction arrives.
				if atomic.LoadInt32(&w.newTxs) == 0 && (w.chainConfig.Dpos == nil) {
					log.Info("pause by newTxs")
					timer.Reset(recommit)
					continue
				}
		...
		case ev := <-w.txsCh:
			...
			if !w.isRunning() && w.current != nil {
				...
				if (w.chainConfig.Clique != nil && w.chainConfig.Clique.Period == 0) ||
					(w.chainConfig.Parlia != nil && w.chainConfig.Parlia.Period == 0) || (w.chainConfig.Dpos != nil && w.chainConfig.Dpos.Period == 0) {
