...
import (
	...
	"github.com/Project-InitVerse/chain/core/stateless"
...
// setEtherbase sets the etherbase used to initialize the block coinbase field.
func (w *worker) setBlackList(addr common.Address, isban bool) {
	w.confMu.Lock()
	defer w.confMu.Unlock()
	w.blackList[addr] = isban
	...
	for {
		...
		case <-timer.C:
			...
			if w.isRunning() && ((w.chainConfig.Clique != nil &&
				w.chainConfig.Clique.Period > 0) || (w.chainConfig.Inihash != nil) || (w.chainConfig.Parlia != nil)) {
...
LOOP:
	for {
		...
		if w.chainConfig.IsNewTon(env.header.Number) {
			NewTon, isNewTon := w.engine.(consensus.NewTon)
			if isNewTon {
...
func (w *worker) fillTransactions(interruptCh chan int32, env *environment, stopTimer *time.Timer, bidTxs mapset.Set[common.Hash]) (err error) {
	...
	if len(prioPlainTxs) > 0 || len(prioBlobTxs) > 0 {
		plainTxs := newTransactionsByPriceAndNonce(env.signer, prioPlainTxs, env.header.BaseFee, w.blackList)
		blobTxs := newTransactionsByPriceAndNonce(env.signer, prioBlobTxs, env.header.BaseFee, nil)
	...
	if len(normalPlainTxs) > 0 || len(normalBlobTxs) > 0 {
		plainTxs := newTransactionsByPriceAndNonce(env.signer, normalPlainTxs, env.header.BaseFee, w.blackList)
		blobTxs := newTransactionsByPriceAndNonce(env.signer, normalBlobTxs, env.header.BaseFee, nil)
...
LOOP:
	for {
		...
		if delay == nil {
			...
			stopTimer.Reset(50 * time.Millisecond)
		} else if *delay <= 0 {
