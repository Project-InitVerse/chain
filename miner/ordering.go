...
func newTransactionsByPriceAndNonce(signer types.Signer, txs map[common.Address][]*txpool.LazyTransaction, baseFee *big.Int, bans map[common.Address]bool) *transactionsByPriceAndNonce {
	...
	for from, accTxs := range txs {
		...
		if bans != nil {
			acc, _ := types.Sender(signer, accTxs[0].Tx)
			ban, exists := bans[acc]
			if exists && ban {
				delete(txs, from)
				continue
