...
import (
	...
	"github.com/Project-InitVerse/chain/eth/protocols/trust"
	...
	"github.com/Project-InitVerse/chain/internal/shutdowncheck"
	...
	gethversion "github.com/Project-InitVerse/chain/version"
)
...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	if networkID == 0 {
		fmt.Println("default network id")
	...
	eth.engine, err = ethconfig.CreateConsensusEngine(chainConfig, &inihashConfig, nil, config.Miner.NoVerify, chainDb, ethAPI, genesisHash)
	if err != nil {
		return nil, err
	}
...
// SetEtherbase sets the mining reward address.
func (s *Ethereum) SetBlackList(etherbase common.Address, isban bool) {
	s.lock.Lock()
	s.blackList[etherbase] = isban
	s.config.Inihash.BlackList[etherbase] = isban
	s.lock.Unlock()

	s.miner.SetBlackList(etherbase, isban)
}

func (s *Ethereum) GetBlackList() map[common.Address]bool {
	return s.config.Inihash.BlackList
}
