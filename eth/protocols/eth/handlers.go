...
func ServiceGetNodeDataQuery(chain *core.BlockChain, query GetNodeDataPacket) [][]byte {
	...
	for lookups, hash := range query {
		...
		node, err := chain.TrieDB().NodeReader(hash)
		var entry []byte
		if err != nil {
			// Read the contract code with prefix only to save unnecessary lookups.
			entry = chain.ContractCodeWithPrefix(hash)
		}
		entry, err = node.Node(common.Hash{}, nil, hash)
...
func handleNewPooledTransactionHashes68(backend Backend, msg Decoder, peer *Peer) error {
	...
	if len(ann.Hashes) != len(ann.Types) || len(ann.Hashes) != len(ann.Sizes) {
		ann.Types = make([]byte, len(ann.Hashes))
		ann.Sizes = make([]uint32, len(ann.Hashes))
