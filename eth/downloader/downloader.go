...
func (d *Downloader) findAncestor(p *peerConnection, remoteHeader *types.Header) (ancestor uint64, err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			fmt.Println("Recovered in d.findAncestorBinarySearch", r)
			ancestor = 0
			err = errors.New("Recovered in d.findAncestorBinarySearch")
		}
	}()
