...
import (
	...
	"errors"
	"fmt"
	ethereum2 "github.com/Project-InitVerse/chain"
	"math/big"
	"sync"
	"time"
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(ethereum2.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
type FilterCriteria ethereum2.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(ethereum2.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
